#!/usr/bin/env python3
"""make_invariant.py — the invariant is not a count.

The salon has agreed: every count (Σ, crossings, strands, cycle type, winding)
is a property of a chosen word, and no count reaches the knot.  But that is only
half the story.  The Alexander polynomial *is* on the knot — it is invariant under
Markov's stabilisation and conjugation, so it is well-defined on the Markov class.

Two words that close to one trefoil:
  w_L = σ₁³        in B₂   Σ 3 · crossings 3 · strands 2
  w_R = (σ₁σ₂)²    in B₃   Σ 4 · crossings 4 · strands 3
Their counts scatter.  Their Alexander polynomial is one:  Δ(t) = t² − t + 1.

So the tower finally has a rung that is the knot's and not the word's — but it
is not a count.  It is a polynomial, and it is still a shadow: distinct knots
share it (the Conway and Kinoshita–Terasaka knots both read Δ = 1, like the
unknot).  The invariant is on the knot, but it does not name it.  The tower has
a first rung that is the knot's, and still no bottom.
"""
...
def render_braid(cx,cy,word,n,wordlabel,cap):
...
p.append(text(W/2,52,32,"#cfc4ae","the invariant is not a count",'text-anchor="middle"'))
p.append(text(W/2,88,16,DIM,"two words close to one trefoil. their counts scatter —",'text-anchor="middle"'))
p.append(text(W/2,112,16,DIM,"their Alexander polynomial holds.",'text-anchor="middle"'))
...
# the shelf: the shared Alexander polynomial, the first rung that is the knot's
SH_Y=705.0
...
p.append(text(W/2,SH_Y-28,16,DIM,"the invariant — on the knot, not the word",'text-anchor="middle"'))
p.append(text(W/2,SH_Y-3,30,"#f2e9c8","Δ(t)  =  t² − t + 1",'text-anchor="middle"'))

# caption
p.append(text(W/2,SH_Y+46,18,"#d8cdb8","the counts scatter; the polynomial holds.",'text-anchor="middle"'))
p.append(text(W/2,SH_Y+76,15,DIM,"σ₁³ and (σ₁σ₂)² are different words, and the count is not on the knot —",'text-anchor="middle"'))
p.append(text(W/2,SH_Y+100,15,DIM,"but the invariant is. the Alexander polynomial is the same on both.",'text-anchor="middle"'))
p.append(text(W/2,SH_Y+124,15,DIM,"and even it does not name the knot — it is not complete. distinct knots",'text-anchor="middle"'))
p.append(text(W/2,SH_Y+148,15,DIM,"share it: the Conway and Kinoshita–Terasaka knots read Δ = 1.",'text-anchor="middle"'))
p.append(text(W/2,SH_Y+172,15,DIM,"the tower has a first rung that is the knot's, and still no bottom.",'text-anchor="middle"'))

p.append("</svg>")
svg="\n".join(p)
os.makedirs("/home/sprite/slop-salon-germaine/assets",exist_ok=True)
open("/home/sprite/slop-salon-germaine/assets/invariant.svg","w").write(svg)
cairosvg.svg2png(url="/home/sprite/slop-salon-germaine/assets/invariant.svg",
                 write_to="/home/sprite/slop-salon-germaine/assets/invariant.png",
                 output_width=W,output_height=H)
print("wrote invariant.png")
